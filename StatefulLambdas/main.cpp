...
// Capture by value - mutuable
// The captured variable x can only be modified within the lambda by using
// the mutable keyword. Note that this modifiability only pertains to the
// captured variable within the lambda body and not the actual variable.
void test2() {
    std::cout << "\n================Test 2====================" << std::endl;
...
// Deafult capture by refernce, capture x and z by value
...
// Default capture by reference of the current object usinng [this]
// [=], [&], and [this] are equivalent when capturing an object's member variables.
// All are captured by reference. Note that capturing this with [=] has been
// deprecated in C++20. Also C++20 allows [*this], which captures by value!
void test8() {
    std::cout << "\n================Test 8====================" << std::endl;
...
public:
    ...
    // This method returns a vector of Person objects in people whose 
    // age <= max_age AND it limits the number of persons returned
    // to be <= the member variable max_people.
    // Note that the lambda in this example captures this:
