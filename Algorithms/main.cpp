...
void find_test() {
    ...
    // Calling the find() function (algorithm)
    auto loc = std::find(std::begin(vec), std::end(vec), 1);

    // Note: loc is an iterator to a vector of integers.
    // We use auto to keep the code short and simple.
...
// See if elements in a container match a predicate
void all_of_test() {
    std::cout << "\n===================================" << std::endl;
